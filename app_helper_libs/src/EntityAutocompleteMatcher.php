...
namespace Drupal\app_helper_libs;
    ...
    if (isset($string)) {
      ...
      foreach ($entity_labels as $values) {
        foreach ($values as $entity_id => $label) {
          $langcode = db_query("SELECT langcode FROM {taxonomy_term_field_data} WHERE tid = :tid", array(':tid' => $entity_id))->fetchField();
          $language_name = ($langcode) ? $languages[$langcode]->getName() : '';
          $key = ($language_name) ? "$label  ($entity_id) [$language_name]" : "$label  ($entity_id)";
          ...
          $key = Tags::encode($key);
          if ($langcode) {
              $label = $label . ' (' . $entity_id . ') [' . $language_name . ']';
          }
