 ...
 */
class PopularTopicSettingsForm extends ConfigFormBase {
  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'popular_topic_admin_settings';
  }
 ...
 /**
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $seeAllPopularTopic = $config->get('see_all_popular_topic.'.$selected);
	...
	*/  
	...
	foreach ($termObjects as $tid => $termObject) {
     ...
     //Get the selected topics from Configuration
      $defaultArr = $config->get('popular_topic.'.$selected); 
      ...
      if (isset($input['_drupal_ajax']) && $input['_drupal_ajax'] == 1) {
        ...
        $routname = 'apptags.see_all_popular_topic_default_settings';
        $url = Url::fromRoute($routname);
        $seeallUrl = \Drupal::l(t('See all topic'), $url);
...
public function submitForm(array &$form, FormStateInterface $form_state) {
  ...
  $this->config('app_helper_libs.settings')
       ->set('popular_topic.'.$lang, $defaultArr)
       ->save();
