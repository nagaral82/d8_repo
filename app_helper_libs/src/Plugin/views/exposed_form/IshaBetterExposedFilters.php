<?php
namespace Drupal\app_helper_libs\Plugin\views\exposed_form;
 ...
 * @ViewsExposedForm(
 *   id = "app_bef",
 *   title = @Translation("app Better Exposed Filters"),
 *   help = @Translation("Provides additional options for exposed form elements.")
 * )
 */
class appBetterExposedFilters extends ExposedFormPluginBase {
  ...
  protected function defineOptions() {
	...
	$options['remember_sort'] = ['default' => FALSE];
    $options['remember_sort_roles'] = array(
      'default' => array(
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
	...
	$bef_options['sort']['advanced']['remember_sort'] = array(
      '#type' => 'checkbox',
     '#title' => $this->t('Remember the last sort selection'),
     '#description' => $this->t('Enable to remember the last sort selection for authenticated users.'),
      '#default_value' => !empty($existing['sort']['advanced']['remember_sort']),
    );

    $role_options = array_map('\Drupal\Component\Utility\Html::escape', user_role_names(FALSE));
    $bef_options['sort']['advanced']['remember_sort_roles'] = array(
      '#type' => 'checkboxes',
      '#title' => $this->t('User roles'),
      '#description' => $this->t('Remember sort selection only for the selected user role(s). If you select no roles, the exposed data will never be stored.'),
      '#default_value' => $existing['sort']['advanced']['remember_sort_roles'],
      '#options' => $role_options,
      '#states' => array(
        'invisible' => array(
          ':input[name="exposed_form_options[remember_sort]"]' => array('checked' => FALSE),
        ),
      ),
    );
...
Off|No
  ...
  public function exposedFormAlter(&$form, FormStateInterface $form_state) {
	...
	if (isset($user_input['_wrapper_format']) && $user_input['_wrapper_format'] == 'drupal_ajax') {
		$user_input = \Drupal::request()->request->all();
	}
	
	//print_r($user_input);
	$query_params = \Drupal::request()->query->all();
  \Drupal::logger('$user_input')->debug('<pre>'.print_r($user_input, TRUE).'</pre>');
    if (isset($user_input['sort_bef_combine'])) {
        // Save new sort value.
        $this->storeSort($user_input['sort_bef_combine'], $user_input);
		//$form_state->setValue('sort_bef_combine', $user_input['sort_bef_combine']);
    }
      ...
      if ($settings['sort']['advanced']['combine']) {
        ...
        foreach ($form['sort_by']['#options'] as $by_key => $by_val) {
          foreach ($form['sort_order']['#options'] as $order_key => $order_val) {
			...
			if ($data = $this->getStoredData()) {
				if (!empty($data['bef_sort_value'])) {
					$selected = $data['bef_sort_value'];
				}
			}
        ...
        $form['#submit'][] = 'app_helper_libs_bef_sort_combine_submit';
  ...
  protected function bef_get_settings() {
    ...
    /**
   * Check if need to save sort data.
   *
   * @return bool
   *   Remember sort result.
   */
  protected function acceptStoreSort() {
	$existing = $this->bef_get_settings();
    if (empty($existing['sort']['advanced']['remember_sort'])) {
      return FALSE;
    }
    // Check if we store sort value for current user.
    $user = \Drupal::currentUser();
    $allowed_rids = empty($existing['sort']['advanced']['remember_sort_roles']) ? array() : array_filter($existing['sort']['advanced']['remember_sort_roles']);
   ...
   *
   * @param string $order
   *   The currently order value to save.
   * @param string $sort
   *   The currently sort value to save.
   */
  protected function storeSort($sort_bef_combine, $query_params = array()) {
    if ($this->acceptStoreSort()) {
      $session = &$this->getStoredData();
