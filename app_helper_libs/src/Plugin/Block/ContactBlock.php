 ...
 * @Block(
 *  id = "contact_block",
 *  admin_label = @Translation("app Contact block"),
 * )
 */
class ContactBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  protected function blockAccess(AccountInterface $account) {
	...
	$selected_domain = $this->configuration['domain_select'];
  ...
  public function blockForm($form, FormStateInterface $form_state) {
	...
	foreach (\Drupal::service('domain.loader')->loadMultipleSorted() as $domain) {
	  $countries[$domain->id()] = $domain->label();
    }
	...
	$form['domain_select'] = [
		...
		'#title' => $this->t('Domain'),
		'#options' => $countries,
		'#default_value' => $this->configuration['domain_select'],
	];
    return $form;
  }
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
	...
	$this->configuration['domain_select'] = $form_state->getValue('domain_select');
  }
